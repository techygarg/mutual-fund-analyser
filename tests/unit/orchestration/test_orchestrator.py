...
class TestAnalysisOrchestrator:
    ...
    def test_single_analysis_execution_flow(self, mock_config_provider: Mock):
        ...
        with patch.object(
            AnalyzerFactory, "create_analyzer", return_value=mock_analyzer
            ...
            with patch.object(orchestrator, "_scrape_and_save_data") as mock_scrape:
                ...
                mock_analyzer.analyze.assert_called_once_with(
                    {"file_paths": {"test": ["/path/to/file.json"]}}, "20240903"
                )
    ...
    def test_scrape_and_save_data_flow(self, mock_config_provider: Mock):
        ...
        mock_coordinator.scrape_for_requirement.return_value = {
            "file_paths": {"test": ["/test/file.json"]}
        }

        with patch.object(
            ScrapingCoordinatorFactory, "create_coordinator", return_value=mock_coordinator
        ) as mock_create_coordinator:
    ...
    def test_error_handling_in_analysis_execution(self, mock_config_provider: Mock):
        ...
        with patch.object(AnalyzerFactory, "create_analyzer", return_value=mock_analyzer):
            ...
            with pytest.raises(
                OrchestrationError, match="Unexpected error during analysis 'holdings'"
            ):
                orchestrator._run_single_analysis("holdings", mock_analysis_config, "20240903")
    ...
    def test_list_available_analyses(self, mock_config_provider: Mock):
        ...
        with patch.object(
            AnalyzerFactory, "get_available_types", return_value=["fund-holdings", "portfolio"]
        ) as mock_get_types:
