...
from mfa.core.exceptions import FactoryError
...
class TestScrapingCoordinatorFactory:
    ...
    def test_create_coordinator_returns_correct_type(self, mock_config_provider: Mock):
        ...
        coordinator = ScrapingCoordinatorFactory.create_coordinator("categories", mock_config_provider)
        ...
        mock_config.analyses = {
            "holdings": Mock(
                data_requirements=Mock(
                    categories={"test": ["https://example.com"]}
                )
            )
        }
