...
class TestPortfolioAnalysis:
    ...
    def _verify_portfolio_config(self, analysis_config) -> None:
        ...
        assert hasattr(analysis_config, "data_requirements"), (
            "Portfolio config should have data_requirements"
        )
    ...
    def _verify_portfolio_results(self, validated_data) -> None:
        ...
        for data in validated_data:
            ...
            assert len(data["funds"]) == portfolio_summary["fund_count"], (
                "Funds array should match fund_count"
            )
            assert len(data["company_allocations"]) == portfolio_summary["unique_companies"], (
                "Companies should match unique_companies"
            )
